...
const isReviewer = (role: string) => role === 'hr';
...
export const compensationRoutes = new Hono()
  ...
  .post('/salary/advances', async (c) => {
    ...
    if (!parsed.success)
      return c.json(
        { message: parsed.error.issues[0]?.message ?? 'Invalid advance request.' },
        400,
      );
  ...
  .post('/loans', async (c) => {
    ...
    if (!parsed.success)
      return c.json({ message: parsed.error.issues[0]?.message ?? 'Invalid loan request.' }, 400);
  ...
  .patch('/review/advances/:id', async (c) => {
    ...
    if (!reviewer) return c.json({ message: 'Unauthorized.' }, 401);
    if (!isReviewer(reviewer.role)) return c.json({ message: 'Only HR can review requests.' }, 403);
    ...
    const advance = await prisma.salaryAdvanceRequest
      ...
      .catch(() => null);
    if (!advance) return c.json({ message: 'Advance request not found.' }, 404);
