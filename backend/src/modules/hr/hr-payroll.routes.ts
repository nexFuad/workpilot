...
function payrollMonthLabel(date: Date) {
...
function allowedPayrollMonth(value: string | undefined) {
...
function exactDateRange(value: string) {
...
export const hrPayrollRoutes = new Hono()
  .get('/', async (c) => {
    ...
    const month =
      allowedPayrollMonth(c.req.query('month')) ?? payrollMonthLabel(previousPayrollMonth());
  ...
  .post('/', async (c) => {
    ...
    if (!result.success) return c.json({ message: 'Invalid payroll data.' }, 400);
  ...
  .patch('/:id/status', async (c) => {
    ...
    if (!result.success) return c.json({ message: 'Select a valid payroll status.' }, 400);
