...
export const hrLoansRoutes = new Hono()
  .get('/', async (c) => {
    ...
    if (!u || u.role !== 'hr') return c.json({ message: 'Unauthorized.' }, 403);
  ...
  .patch('/:id/status', async (c) => {
    ...
    if (!d.success) return c.json({ message: 'Invalid status.' }, 400);
    const r = await prisma.loanRequest
      ...
      .catch(() => null);
    if (!r) return c.json({ message: 'Request not found.' }, 404);
