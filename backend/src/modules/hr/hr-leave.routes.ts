...
function monthRange(year: number, month: number): DateRange | null {
...
function searchDateRange(value: string): DateRange | null {
...
export const hrLeaveRoutes = new Hono()
  ...
  .patch('/:id/status', async (c) => {
    ...
    if (!data.success) return c.json({ message: 'Select approve or reject.' }, 400);
