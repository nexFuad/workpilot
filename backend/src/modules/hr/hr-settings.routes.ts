...
async function reviewer(c: Context) {
  const user = await currentUser(c);
  return user?.role === 'hr' ? user : null;
}
export const hrSettingsRoutes = new Hono()
  .get('/sites', async (c) => {
    if (!(await reviewer(c))) return c.json({ message: 'Unauthorized.' }, 403);
  ...
  .post('/sites', async (c) => {
    ...
    if (!data.success) return c.json({ message: 'Invalid site information.' }, 400);
  ...
  .delete('/sites/:id', async (c) => {
    ...
    return site
      ...
      : c.json(
          { message: 'Site cannot be deleted because it is used by attendance records.' },
          409,
        );
  })
  .get('/shifts', async (c) => {
  ...
  .post('/shifts', async (c) => {
    ...
    if (!data.success) return c.json({ message: 'Invalid shift information.' }, 400);
  ...
  .delete('/shifts/:id', async (c) => {
    ...
    return shift
      ...
      : c.json(
          { message: 'Shift cannot be deleted because it is used by attendance records.' },
          409,
        );
  });
