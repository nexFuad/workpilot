...
export const documentRoutes = new Hono()
  ...
  .get('/review', async (c) => {
    ...
    if (reviewer.role !== 'hr') return c.json({ message: 'Only HR can review documents.' }, 403);
  ...
  .patch('/:id/review', async (c) => {
    ...
    if (!parsed.success) return c.json({ message: 'Invalid review.' }, 400);
  ...
  .delete('/:id', async (c) => {
    const reviewer = await currentUser(c);
    if (!reviewer) return c.json({ message: 'Unauthorized.' }, 401);
    if (reviewer.role !== 'hr') return c.json({ message: 'Only HR can delete documents.' }, 403);
    const document = await prisma.employeeDocument
