...
const timeValue = (date) => date.getHours() * 60 + date.getMinutes();
const clock = (value) => {
...
export const hrAttendanceRoutes = new Hono().get('/', async (c) => {
    ...
    const attendance = employees.map((employee) => {
        ...
        const overtimeMinutes = record.checkOutAt
            ? Math.max(timeValue(record.checkOutAt) -
                clock(record.checkOutShift?.endTime ?? record.checkInShift.endTime), 0)
            : 0;
        return {
            employee,
