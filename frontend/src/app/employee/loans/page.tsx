...
export default function LoansPage() {
  ...
  const { loans, createLoan } = useCompensation();
  const apiLoan = loans.data?.loans.find((loan) => loan.status === 'active');
  const activeLoan = apiLoan ? { ...apiLoan, remaining: apiLoan.outstanding } : currentLoan;
  ...
  const paidAmount = activeLoan.principal - activeLoan.remaining;
  const progress = (paidAmount / activeLoan.principal) * 100;
    ...
    <section className="w-full space-y-6 pb-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <article className="rounded-2xl border border-sky-100 bg-gradient-to-br from-sky-600 to-cyan-500 p-5 text-white shadow-sm shadow-sky-200">
          ...
          <p className="mt-5 text-2xl font-bold">{currency.format(currentLoan.remaining)}</p>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-5 text-2xl font-bold text-slate-800">
            {currency.format(currentLoan.installment)}
          </p>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-5 text-xl font-bold text-slate-800">{currentLoan.nextDue}</p>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 rounded-xl bg-slate-50 p-4">
            <div className="flex justify-between text-sm">
              <span className="font-semibold text-slate-800">
                {currentLoan.paidInstallments} of {currentLoan.tenure} instalments paid
              </span>
            ...
            <div className="mt-4 flex justify-between text-sm">
              ...
              <span className="font-semibold text-slate-800">
                Remaining: {currency.format(currentLoan.remaining)}
              </span>
            </div>
          </div>
          <div className="mt-5 divide-y divide-slate-100 text-sm">
            <div className="flex justify-between py-3">
              ...
              <span className="font-semibold text-slate-800">
                {currency.format(currentLoan.principal)}
              </span>
            </div>
            <div className="flex justify-between py-3">
            ...
            <div className="flex justify-between py-3">
              ...
              <span className="font-semibold text-slate-800">{currentLoan.nextDue}</span>
            </div>
          </div>
        </article>
